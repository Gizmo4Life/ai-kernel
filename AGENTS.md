...
glossary definitions, coding standards, skills, instructions, and agent definitions.
...
1. [Glossary](glossary/glossary-entry.glossary.md): Before defining a concept inline, check `glossary/`.
   If a definition exists, link to it. If not, ask [flynn.agent](agents/flynn.agent.md) for guidance on whether to create a new definition or inline it.
2. [Standards](glossary/standard.glossary.md): When working in a scoped context (e.g. C++ code), load the relevant standard from `standards/` and validate
   output against its [PADU table](glossary/padu-scale.glossary.md). Standards should glob on file extensions and relative paths to target files for context (e.g. the .md files in /standards/ are the context for the [Standard File Standard](standards/standard-file.standard.md)).
3. [Skills](glossary/skill.glossary.md): Atomic actions are defined in `skills/`. Read the
   [frontmatter.glossary](glossary/frontmatter.glossary.md) to find relevant skills, then load the body when needed, the body defines how to effectively utilize a tool or apply a skill.
...
All content files use YAML [frontmatter.glossary](glossary/frontmatter.glossary.md). To find relevant files:
